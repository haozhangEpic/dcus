...
export default function LimitVirtualList(): JSX.Element {
  ...
  const pageSize = 10; // 原始数据源
  const [dataSource, setDataSource] = useState([]);
  const scrollRef = createRef<HTMLDivElement>();
  const [totalCount, setTotalCount] = useState(0);
  const [beforeCount, setBeforeCount] = useState(0);
  const [pageNum, setPageNum] = useState(1); // 真正渲染的数据
  ...
  useEffect(() => {
    sliceShowDataSource();
  }, [pageNum, dataSource.length]); // 数据初始化
  const init = () => {
  ...
  }; // 1、监听用户scroll事件 // 2、实时计算页码 // 3、如果页码发生改变，进行数据切片，重新渲染数据 // 4、如果页码没有发生改变，保持不动
  const onScroll = () => {
    ...
    const currPageNum = Math.min(scrollPageNum, maxPageNum); // 如果当前页数保持不变
  ...
  const getRenderData = ({ pageNum, pageSize, dataSource }) => {
    const startIndex = (pageNum - 1) * pageSize; // 这里+2：想要保证顺畅的滑动，快速滑动不白屏，需要至少预留3页数据，前+中+后
    const endIndex = Math.min((pageNum + 2) * pageSize, dataSource.length);
    return {
      showDataSource: dataSource.slice(startIndex, endIndex), // 前置数量
      beforeCount: startIndex,
      totalCount: dataSource.length,
    };
  };
  return (
    <div className={styles.limitvirtuallist}>
      {/* 容器层：固定高度 */}
        ...
        <div style={{ height: `${totalCount * itemHeight}px` }}>
          ...
          >
            ...
            {showDataSource.map((item, index) => {
              return (
                <div className={styles.scroll_item} key={index}>
                  <p>{item.words}</p>
                  <p>{item.content}</p>
                </div>
              );
